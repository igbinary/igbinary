...
#if PHP_VERSION_ID >= 70200
		/* If the visibility of a property has changed between serialization and unserialization, or if a private
		 * or protected property needs to be restored, but the serialized data was generated by a __serialize method
		 * while no __unserialize method exists in the class, the property value cannot be found during hydration
		 * in the object's hash table. The reason for this is that in these cases the property name in the serialized
		 * representation does not match the prefixed/mangled property name of the current class definition.
		 * To resolve this, an attempt is made to determine the correctly prefixed property name by performing a lookup
		 * with the unprefixed property name in the properties_info hash table of the class. This aligns the behavior
		 * to the serializer of PHP >= 7.2. */
		if (prototype_value == NULL) {
			zend_property_info *fallback_info = NULL;
			const char *unmangled_class = NULL;
			const char *unmangled_prop;
			size_t unmangled_prop_len;

			if (UNEXPECTED(zend_unmangle_property_name_ex(key_str, &unmangled_class, &unmangled_prop, &unmangled_prop_len) == FAILURE)) {
				zend_string_release(key_str);
				return 1;
			}

			if (unmangled_class == NULL) {
				fallback_info = zend_hash_find_ptr(&ce->properties_info, key_str);
			} else if (!strcmp(unmangled_class, "*") || !strcasecmp(unmangled_class, ZSTR_VAL(ce->name))) {
				fallback_info = zend_hash_str_find_ptr(&ce->properties_info, unmangled_prop, unmangled_prop_len);
			}

			if (fallback_info != NULL) {
#if PHP_VERSION_ID >= 80400
				if ((fallback_info->flags & ZEND_ACC_VIRTUAL)) {
					php_error_docref(NULL, E_WARNING,
						"Cannot unserialize value for virtual property %s::$%s",
						ZSTR_VAL(fallback_info->ce->name), unmangled_prop);
					zend_string_release(key_str);
					return 1;
				}
#endif
				zend_string_release(key_str);
				key_str = zend_string_copy(fallback_info->name);
				prototype_value = zend_hash_find(h, key_str);
			}
		}
#endif
